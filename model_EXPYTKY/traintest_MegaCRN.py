...
def evaluateModel(model, data_iter):
    ...
    if opt.loss == 'MaskMAE': criterion = masked_mae
...
parser.add_argument('--seq_len', type=int, default=6, help='sequence length of prediction')
parser.add_argument('--his_len', type=int, default=6, help='sequence length of historical observation')
...
def main():
    ...
    speed_data = [data[:,:,0] for data in train_data+test_data]
    speed_data = np.vstack(speed_data)    
    scaler.fit(speed_data)
    for data in train_data + test_data:
        data[:,:,0] = scaler.transform(data[:,:,0])
    
    logger.info(opt.dataset, opt.month, 'training started', time.ctime())
    trainXS, trainYS = getXSYS(train_data, opt.his_len, opt.seq_len)
    _, trainYCov = getXSYS(train_time, opt.his_len, opt.seq_len)
    ...
    logger.info(opt.dataset, opt.month, 'training ended', time.ctime())
    logger.info('=' * 90)
    
    logger.info(opt.dataset, opt.month, 'testing started', time.ctime())
    testXS, testYS = getXSYS(test_data, opt.his_len, opt.seq_len)
    _, testYCov = getXSYS(test_time, opt.his_len, opt.seq_len)
    ...
    logger.info(opt.dataset, opt.month, 'testing ended', time.ctime())
    logger.info('=' * 90)
