...
def main():
    ...
    parser.add_argument('--num_variable', type=int, default=1843, help='number of variables (e.g., 1843 road links in EXPY-TKY)')
    ...
    model = MegaCRN(num_nodes=opt.num_variable, input_dim=opt.channelin, output_dim=opt.channelout, horizon=opt.seq_len, rnn_units=opt.hiddenunits, memory_type=opt.memory, meta_type=opt.meta, decoder_type=opt.decoder).to(device)
    summary(model, [(opt.his_len, opt.num_variable, opt.channelin), (opt.seq_len, opt.num_variable, opt.channelout)], device=device)
    print_params(model)
