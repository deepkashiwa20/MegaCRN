...
class AGCN(nn.Module):
    ...
    def forward(self, x, node_embeddings):
        # inputed node_embeddings are 2D, it will never go to else.
...
class MegaCRN(nn.Module):
    ...
    def construct_memory(self):
        ...
        memory_dict['We'] = nn.Parameter(torch.randn(self.embed_dim, self.mem_dim), requires_grad=True)    # project to query
    ...
    def query_memory(self, h_t:torch.Tensor):
        ...
        query_e = torch.matmul(self.node_embeddings, self.memory['We'])  # (N,e) * (e,d) -> (N, d)
        att_score_e = torch.softmax(torch.matmul(query_e, self.memory['Memory'].t()), dim=-1)   # alpha: (N, M)
        _, ind_e = torch.topk(att_score_e, k=1, dim=-1)
        pos_e = self.memory['Memory'][ind_e[:, 0]] # (N, d)
        # print('query_e.shape, att_score_e.shape, ind_e.shape, pos_e.shape', query_e.shape, att_score_e.shape, ind_e.shape, pos_e.shape)
        return pos_e, proto, query, pos, neg
...
def main():
    ...
    model = MegaCRN(num_nodes=args.num_variable, input_dim=args.channelin, output_dim=args.channelout, 
                    horizon=args.seq_len, rnn_units=args.rnn_units, memory_type=args.memory, meta_type=args.meta).to(device)
