...
class MegaCRN(nn.Module):
    ...
    def construct_memory(self):
        ...
        memory_dict['We'] = nn.Parameter(torch.randn(self.num_nodes, self.mem_num), requires_grad=True) # project memory to embedding
    ...
    def query_memory(self, h_t:torch.Tensor):
        ...
        e_t = torch.matmul(self.memory['We'], self.memory['Memory']) # (N, M) * (M, d) = (N, d)
        return e_t, proto_t, query, pos, neg
            
    def forward(self, x, y_cov, labels=None, batches_seen=None):       
        init_state = self.encoder.init_hidden(x.shape[0])
        node_embeddings = torch.matmul(self.memory['We'], self.memory['Memory'])
        h_en, state_en = self.encoder(x, init_state, node_embeddings) # B, T, N, hidden
