...
def evaluate(model, mode):
    with torch.no_grad():
        ...
        for x, y in data_iter:
            ...
            separate_loss = nn.TripletMarginLoss(margin=args.margin)
        ...
        return mean_loss, mean_loss1, mean_loss2, mean_loss3, ys_true, ys_pred
...
def traintest_model():  
    ...
    for epoch_num in range(args.epochs):
        ...
        train_loss1 = np.mean(losses1)
        train_loss2 = np.mean(losses2)
        train_loss3 = np.mean(losses3)
        lr_scheduler.step()
        val_loss, val_loss1, val_loss2, val_loss3, _, _ = evaluate(model, 'val')
        # if (epoch_num % args.test_every_n_epochs) == args.test_every_n_epochs - 1:
        end_time2 = time.time()
        message1 = 'Epoch [{}/{}] ({}), lr: {:.6f}, {:.1f}s, train_loss: {:.4f}, train_loss1: {:.4f}, train_loss2: {:.4f}, train_loss3: {:.4f}, '.format(epoch_num + 1, 
                   args.epochs, batches_seen, optimizer.param_groups[0]['lr'], (end_time2 - start_time), train_loss, train_loss1, train_loss2, train_loss3)
        message2 = 'Epoch [{}/{}] ({}), lr: {:.6f}, {:.1f}s, val_loss: {:.4f}, val_loss1: {:.4f}, val_loss2: {:.4f}, val_loss3: {:.4f}, '.format(epoch_num + 1, 
                   args.epochs, batches_seen, optimizer.param_groups[0]['lr'], (end_time2 - start_time), val_loss, val_loss1, val_loss2, val_loss3)
        logger.info(message1)
        logger.info(message2)
        test_loss, test_loss1, test_loss2, test_loss3, _, _ = evaluate(model, 'test')
        end_time2 = time.time()
        message3 = 'Epoch [{}/{}] ({}), lr: {:.6f}, {:.1f}s, test_loss: {:.4f}, test_loss1: {:.4f}, test_loss2: {:.4f}, test_loss3: {:.4f}, '.format(epoch_num + 1, 
                   args.epochs, batches_seen, optimizer.param_groups[0]['lr'], (end_time2 - start_time), test_loss, test_loss1, test_loss2, test_loss3)
...
parser.add_argument('--lamb', type=float, default=1.0, help='lamb value for separate loss')
parser.add_argument('--lamb1', type=float, default=1.0, help='lamb1 value for compact loss')
...
logger.info('triplet margin', args.margin)
