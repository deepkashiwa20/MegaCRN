...
class MegaCRN(nn.Module):
    ...
    def construct_memory(self):
        ...
        memory_dict['We'] = nn.Parameter(torch.randn(self.mem_dim, self.embed_dim), requires_grad=True)
    ...
    def query_memory(self, h_t:torch.Tensor):
        ...
        emb = torch.matmul(proto_t, self.memory['We']) # (B, N, e)
        _, ind = torch.topk(att_score, k=2, dim=-1)
        pos = self.memory['Memory'][ind[:, :, 0]] # B, N, d
        neg = self.memory['Memory'][ind[:, :, 1]] # B, N, d
        return emb, proto_t, query, pos, neg
