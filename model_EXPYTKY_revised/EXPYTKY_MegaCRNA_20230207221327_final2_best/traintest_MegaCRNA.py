...
def evaluateModel(model, data_iter):
    ...
    if args.loss == 'MaskMAE': criterion = masked_mae
...
def trainModel(name, mode, XS, YS, YCov):
    ...
    trainval_iter = torch.utils.data.DataLoader(trainval_data, args.batch_size, shuffle=False)
